...
filtered_treemap_data <- data_cleaned %>%
  filter(
...
```{r}
ggplot(dat, aes(x = `Cohort Year`)) +
  geom_bar(aes(y = `clean_# Total Cohort`), stat = "identity", fill = "lightblue", position = "dodge") +
  geom_bar(aes(y = `clean_# Total Regents`), stat = "identity", fill = "darkblue", alpha = 0.5, position = "dodge") +
  theme_minimal() +
...
```{r}
ggplot(data_cleaned, aes(x = `clean_# Advanced Regents`, y = `clean_% Grads`)) +
...
```{r}
ggplot(data_cleaned, aes(x = `clean_% Still Enrolled`)) +
  geom_density(fill = "lightblue", alpha = 0.5) +
  geom_density(aes(x = `clean_% Dropout`), fill = "darkblue", alpha = 0.5) +
  theme_minimal() +
...
```
```{r}
ggplot(data_cleaned, aes(x = `clean_# TASC (GED)`, y = `clean_# Advanced Regents`, color = `clean_# Regents without Advanced`)) +
  geom_point(alpha = 0.6) +
  labs(title = "TASC vs Regents with Advanced", x = "TASC", y = "Advanced Regents")
...
```{r}
library(treemap)
data_tree <- data_cleaned |> filter(`Report Category` == "Borough")
treemap(data_tree, 
        index = c("Geographic Subdivision", "Category"), 
        vSize = "clean_# Total Cohort", 
        vColor = "clean_%Grads", 
        draw = TRUE)
